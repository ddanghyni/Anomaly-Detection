...
citekey: "{{citekey}}"
aliases:
  - '{%- if creators -%} {{ creators[0].lastName }}{%- if creators|length > 1 -%} et al.{%- endif -%} {%- endif -%} {%- if date -%} ({{ date | format("YYYY") }}){%- endif -%} {%- if shortTitle -%} {{ shortTitle | safe }}{%- else -%} {{ title | safe }}{%- endif -%}'
title: "{{title}}"
authors:
  - "{{authors}}"
tags:
